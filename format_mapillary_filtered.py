...
#                               recursive=True))
csv_files_paths = sorted(glob(join("datasets", "mapillary_sls", "*", "*", "*", "*", "postprocessed_filtered.csv"),
                              recursive=True))

for csv_file_path in csv_files_paths:
    with open(csv_file_path, "r") as file:
        postprocessed_lines = file.readlines()[1:]
    with open(csv_file_path.replace("postprocessed_filtered", "raw_filtered"), "r") as file:
